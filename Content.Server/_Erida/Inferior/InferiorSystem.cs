...
public sealed class InferiorSystem : SharedInferiorSystem
{
    ...
    private void OnImplantImplanted(Entity<InferiorImplantComponent> entity, ref ImplantImplantedEvent ev)
    {
        ...
        EnsureComp<InferiorComponent>(ev.Implanted.Value, out var comp);
        ...
        {
            EnsureComp<NpcFactionMemberComponent>(ev.Implanted.Value, out var nPCImComp);
            comp.OldFactions = [.. nPCImComp.Factions];
            _npcFaction.ClearFactions(ev.Implanted.Value);
            _npcFaction.AddFactions(ev.Implanted.Value, nPCComp.Factions);
        }

        entity.Comp.ImplanterUid = null;
        Start(ev.Implanted.Value, comp);
    }
